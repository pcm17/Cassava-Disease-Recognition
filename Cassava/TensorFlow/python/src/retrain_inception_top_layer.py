...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      ...
      if percentage_hash < validation_percentage and ('TEST' not in file_name):
...
def main(_):
  ...
  tr_batch_size = FLAGS.train_batch_size
  test_percentage = FLAGS.testing_percentage
  num_steps = FLAGS.how_many_training_steps
  
  # Write out the results along with the learning rate, train batch size, number of training steps, and test percentage
  results=open('plantvillage/cassava/resources/original/results/results_files/results_inception.txt', 'a')
  results.write('%.2f\t%.3f\t%d\t%d\t%d\n' % ((test_accuracy*100),lr,tr_batch_size,num_steps,test_percentage))
  
  # write out prediction and true labels to generate confusion matrices in matlab
  targets = []
  for i in range(0, len(test_bottlenecks)):
      targets.append(test_ground_truth[i].argmax())

  a = np.asarray(targets)
  np.savetxt("plantvillage/cassava/resources/original/labels_predictions/"+str(test_percentage)+"/labels.csv", a, delimiter=",")
  
  a = np.asarray(predictions)
  np.savetxt("plantvillage/cassava/resources/original/labels_predictions/"+str(test_percentage)+"/predictions.csv", a, delimiter=",")
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--bottleneck_dir',
      type=str,
      default='/Users/SneakyPT/googledrive/cassava_originals/bottleneck',
